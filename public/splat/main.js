...
async function main() {
  ...
  //   // "nike.splat",
  //   // location.href,
  //   window.defaultURL || params.get("url") || "train.splat",
  //   window.defaultURL && window.defaultURL.startsWith("http")
  //     ? ""
  ...
  if (req.status != 200) {
    const error = new Error(req.status + " Unable to load " + req.url);
  ...
  const frame = (now) => {
    ...
    if (vertexCount > 0) {
      ...
      // Notify parent window that splat is loaded
      if (window.parent !== window) {
        window.parent.postMessage(
          {
            type: "splat-loaded",
